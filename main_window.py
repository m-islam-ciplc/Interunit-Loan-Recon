...
class MainWindow(QMainWindow):
    ...
    def start_matching(self):
        ...
        # Show sleep prevention warning for large files based on row counts
        try:
            import openpyxl
            def get_row_count(file_path):
                # Use read_only=True to quickly get sheet dimensions without loading all data
                wb = openpyxl.load_workbook(file_path, read_only=True, keep_links=False)
                return wb.active.max_row or 0

            row_count1 = get_row_count(self.current_file1)
            row_count2 = get_row_count(self.current_file2)
            
            # Use 1,000 rows as the threshold for "large" processing
            if row_count1 > 1000 or row_count2 > 1000:
                reply = QMessageBox.question(
                    self,
                    "Large File Processing",
                    f"Processing large files ({row_count1:,} and {row_count2:,} rows).\n\n"
                    f"This may take a while. The application will prevent PC sleep during processing.\n\n"
            ...
            pass  # If we can't get row counts, just continue
